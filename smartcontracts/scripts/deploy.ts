...
const images = JSON.parse(fs.readFileSync("./../ipfs.json").toString());

//['QmZWbZ7ogH3ER5EvBFPYfWkpZnY8zZttfUnSWZGgw8mKje', 'QmNNG72zU4qGxP41D1xVuRgkuKBQdgCpY82FSdZYnkDofG'];
...
async function main() {
  ...
  await (await Erc721myInstance.setMinter(NFTMarketplaceInstance.address)).wait();
  await (await Erc1155myInstance.setMinter(NFTMarketplaceInstance.address)).wait();
  
  for (let i = 0; i < 10; i++) {
    console.log(`mint 721 ${i}`, images[i]);
    const tx = await NFTMarketplaceInstance["createItem(string)"](`${images[i]}`, {gasPrice: '68813487855'});
    await tx.wait();
  }
